...
const { connectMQTT } = require('./iot-service/mqtt/mqtt-init.js')
...
const electronConfig = {
  ...
  URL_LAUNCHER_TITLE: process.env.URL_LAUNCHER_TITLE || 'FCS Parking Controller',
  URL_LAUNCHER_CONSOLE: process.env.URL_LAUNCHER_CONSOLE === '1' ? 1 : 0,
  URL_LAUNCHER_URL: process.env.URL_LAUNCHER_URL || `${path.join(__dirname, 'build', 'index.html')}`,
...
app.on('ready', () => {
  ...
  // mainWindow.loadFile(electronConfig.URL_LAUNCHER_URL); for production
  mainWindow.loadURL(electronConfig.URL_LAUNCHER_URL)
})

// Import all the IPC handler from Main Process
